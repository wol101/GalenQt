...
images = {'033r-036v_A_001(F).tif','033r-036v_A_002(F).tif','033r-036v_A_003(F).tif','033r-036v_A_004(F).tif','033r-036v_A_005(F).tif'};

inf = imfinfo(images{1});
cols = length(images);
...
for i = 1: length(images)
    output_file = sprintf('PC%d.tif', i);
    timg = score(:, i);
...
%   Zhang Jiang 
%   $Revision: 1.1 $  $Date: 2018/03/20 08:01:14 $
